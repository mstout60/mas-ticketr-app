    ...
    const peopleAhead = await ctx.db
      ...
      .filter((q) =>
        q.and(
          ...
          q.lt(q.field("_creationTime"), entry._creationTime ?? 0),
          // Only get the entries tht are either waiting or offered
          q.or(
...
export const expireOffer = internalMutation({
  ...
  handler: async (ctx, { waitingListId, eventId }) => {
    ...
    // If offer is not found or is not OFFERED status, do nothing
